...
- 中间平台高度 `1.50m`
- `1` 个更大的不透明二楼平台，高度 `3.00m`
- 闭合外墙、局部室内隔墙、二楼栏杆和支撑柱，让场景更接近室内白盒环境，而不是开放场地

布局上，机器人默认从场地中心附近出发，坡道、台阶、楼梯和墙体分布在不同区域，避免一开始就穿模或压在地形上，也方便从多个方向观察 `/registered_scan`、`/terrain_map` 和 `/terrain_map_ext`。南侧楼梯和北侧候选楼梯连接同一个二楼平台，用来测试“一楼目标”和“二楼目标”是否能通过同一套 `/goal_point` 接口连通。
